...
func GetField(raw []string, bitmap []int) []entity.Field {
	...
	for _, c := range bitmap {
		...
		case 2:
			...
			field.Value, raw, field.Length = TypeVar(raw, 2)

			datel = append(datel, field)
		case 3:
			field3 := AssignField(3, "Processing Code")
			field3.Length = 6
			field3.Value, raw = TypeFixed(raw, field3.Length)

			datel = append(datel, field3)
		case 4:
			field := AssignField(4, "Amount Transaction")
			field.Length = 12
			field.Value, raw = TypeFixed(raw, field.Length)
		...
		case 36:
			...
			field.Value, raw, field.Length = TypeVar(raw, 3)
